...
#!/usr/bin/env python3
"""
XGBoost Ant Detection System - Enhanced Desktop Application
Complete port from Streamlit with XGBoost backbone for improved performance
"""
...
class XGBoostAntDetector:
    """Complete XGBoost Ant Detector - upgraded from Random Forest for better performance"""
    ...
    def __init__(self):
        ...
        # XGBoost parameters (optimized for ant detection)
        self.xgb_params = {
            'n_estimators': min(100, max_estimators),
            'max_depth': 6,  # XGBoost typically uses shallower trees than RF
            'learning_rate': 0.1,
            'subsample': 0.8,
    ...
    def train_model(self, images, annotations, tune_hyperparameters=True, hyperparam_ranges=None, progress_callback=None):
        """Train model with hyperparameter tuning - now using XGBoost"""
        try:
            # Validate inputs
            ...
            if tune_hyperparameters:
                ...
                search = RandomizedSearchCV(
                    ...
                    n_iter=8,  # Slightly more iterations for XGBoost
                    cv=cv_folds,
                    scoring='f1',
                    n_jobs=1,
                    random_state=42,
                ...
                # Add scale_pos_weight to default params
    ...
    def load_model_from_file(self, filepath):
        ...
        try:
            ...
            # Handle both old rf_params and new xgb_params for backward compatibility
            if 'xgb_params' in config:
                self.xgb_params = config.get('xgb_params', self.xgb_params)
            elif 'rf_params' in config:
                # Old model - keep rf_params for compatibility
                self.rf_params = config.get('rf_params', getattr(self, 'rf_params', {}))
...
class AntDetectionApp:
    def __init__(self, root):
        self.root = root
        self.root.title("XGBoost Ant Detection System")
        ...
        # Initialize detector
        self.detector = XGBoostAntDetector()
    ...
    def create_widgets(self):
        ...
        title_text = "🐜 XGBoost Ant Detection System"
...
"""
    ...
    def create_training_tab(self):
        ...
        # Learning rate
        ttk.Label(self.hyperparam_frame, text="Learning Rate:", style='Info.TLabel').grid(row=2, column=0, sticky=tk.W, pady=2)
        lr_frame = ttk.Frame(self.hyperparam_frame)
        lr_frame.grid(row=2, column=1, sticky=(tk.W, tk.E), pady=2)
        
        self.lr_min_var = tk.DoubleVar(value=0.01)
        self.lr_max_var = tk.DoubleVar(value=0.3)
        ttk.Label(lr_frame, text="Min:", style='Info.TLabel').pack(side=tk.LEFT, padx=(0, 2))
        ttk.Entry(lr_frame, textvariable=self.lr_min_var, width=6).pack(side=tk.LEFT, padx=2)
        ttk.Label(lr_frame, text="Max:", style='Info.TLabel').pack(side=tk.LEFT, padx=(10, 2))
        ttk.Entry(lr_frame, textvariable=self.lr_max_var, width=6).pack(side=tk.LEFT, padx=2)
        
        # Minimum child weight
        ttk.Label(self.hyperparam_frame, text="Min Child Weight:", style='Info.TLabel').grid(row=3, column=0, sticky=tk.W, pady=2)
        child_weight_frame = ttk.Frame(self.hyperparam_frame)
        child_weight_frame.grid(row=3, column=1, sticky=(tk.W, tk.E), pady=2)
        
        self.child_weight_min_var = tk.IntVar(value=1)
        self.child_weight_max_var = tk.IntVar(value=7)
        ttk.Label(child_weight_frame, text="Min:", style='Info.TLabel').pack(side=tk.LEFT, padx=(0, 2))
        ttk.Entry(child_weight_frame, textvariable=self.child_weight_min_var, width=6).pack(side=tk.LEFT, padx=2)
        ttk.Label(child_weight_frame, text="Max:", style='Info.TLabel').pack(side=tk.LEFT, padx=(10, 2))
        ttk.Entry(child_weight_frame, textvariable=self.child_weight_max_var, width=6).pack(side=tk.LEFT, padx=2)
        
        # Subsample ratio
        ttk.Label(self.hyperparam_frame, text="Subsample:", style='Info.TLabel').grid(row=4, column=0, sticky=tk.W, pady=2)
        subsample_frame = ttk.Frame(self.hyperparam_frame)
        subsample_frame.grid(row=4, column=1, sticky=(tk.W, tk.E), pady=2)
        
        self.subsample_min_var = tk.DoubleVar(value=0.6)
        self.subsample_max_var = tk.DoubleVar(value=1.0)
        ttk.Label(subsample_frame, text="Min:", style='Info.TLabel').pack(side=tk.LEFT, padx=(0, 2))
        ttk.Entry(subsample_frame, textvariable=self.subsample_min_var, width=6).pack(side=tk.LEFT, padx=2)
        ttk.Label(subsample_frame, text="Max:", style='Info.TLabel').pack(side=tk.LEFT, padx=(10, 2))
        ttk.Entry(subsample_frame, textvariable=self.subsample_max_var, width=6).pack(side=tk.LEFT, padx=2)
        
        # Column sample by tree
        ttk.Label(self.hyperparam_frame, text="Column Sample:", style='Info.TLabel').grid(row=5, column=0, sticky=tk.W, pady=2)
        colsample_frame = ttk.Frame(self.hyperparam_frame)
        colsample_frame.grid(row=5, column=1, sticky=(tk.W, tk.E), pady=2)
        
        self.colsample_min_var = tk.DoubleVar(value=0.6)
        self.colsample_max_var = tk.DoubleVar(value=1.0)
        ttk.Label(colsample_frame, text="Min:", style='Info.TLabel').pack(side=tk.LEFT, padx=(0, 2))
        ttk.Entry(colsample_frame, textvariable=self.colsample_min_var, width=6).pack(side=tk.LEFT, padx=2)
        ttk.Label(colsample_frame, text="Max:", style='Info.TLabel').pack(side=tk.LEFT, padx=(10, 2))
        ttk.Entry(colsample_frame, textvariable=self.colsample_max_var, width=6).pack(side=tk.LEFT, padx=2)
...
• Max Depth: How deep each tree can grow (XGBoost uses shallower trees)
• Learning Rate: Step size for each boosting iteration (lower = more robust)
• Min Child Weight: Minimum sum of weights needed in a child node
• Subsample: Fraction of samples used for each tree (prevents overfitting)
• Column Sample: Fraction of features used for each tree"""
        
        info_label = ttk.Label(self.hyperparam_frame, text=info_text, style='Info.TLabel', justify=tk.LEFT)
        info_label.grid(row=6, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=5)
    ...
    def train_model(self):
        """Train the XGBoost model with comprehensive functionality"""
        if not hasattr(self, 'training_images') or not hasattr(self, 'training_annotations'):
        ...
        if tune_hyperparams:
            ...
            lr_max = self.lr_max_var.get()
            lr_values = [lr_min, (lr_min + lr_max) / 2, lr_max]
            
            # Min child weight values
            child_weight_values = list(range(self.child_weight_min_var.get(), self.child_weight_max_var.get() + 1,
                                            max(1, (self.child_weight_max_var.get() - self.child_weight_min_var.get()) // 2)))
            if len(child_weight_values) > 3:
                child_weight_values = [child_weight_values[0], child_weight_values[len(child_weight_values)//2], child_weight_values[-1]]
            
            # Subsample values
            subsample_min = self.subsample_min_var.get()
            subsample_max = self.subsample_max_var.get()
            subsample_values = [subsample_min, (subsample_min + subsample_max) / 2, subsample_max]
            
            # Column sample by tree values
            colsample_min = self.colsample_min_var.get()
            colsample_max = self.colsample_max_var.get()
            colsample_values = [colsample_min, (colsample_min + colsample_max) / 2, colsample_max]
            
            hyperparam_ranges = {
                'n_estimators': trees_values,
                'max_depth': depth_values,
                'learning_rate': lr_values,
                'min_child_weight': child_weight_values,
    ...
    def create_training_report(self, results):
        ...
        report_text = f"""XGBoost Ant Detection Model Training Report
====================================================
Generated: {timestamp}
...
"""
        ...
        if 'best_params' in results and results['best_params']:
            ...
            report_text += "------------------------\n"
            report_text += f"n_estimators: {self.detector.xgb_params['n_estimators']}\n"
            report_text += f"max_depth: {self.detector.xgb_params['max_depth']}\n"
            report_text += f"learning_rate: {self.detector.xgb_params['learning_rate']}\n"
            report_text += f"subsample: {self.detector.xgb_params['subsample']}\n"
            report_text += f"colsample_bytree: {self.detector.xgb_params['colsample_bytree']}\n"
            report_text += f"random_state: {self.detector.xgb_params['random_state']}\n"
            report_text += f"tree_method: {self.detector.xgb_params['tree_method']}\n"
...
---------------"""
        ...
        report_text += "\n\nNOTES\n-----\n"
        report_text += "This model uses XGBoost gradient boosting classification with comprehensive feature extraction\n"
        report_text += "including HOG, LBP, Gabor filters, and statistical features for robust ant detection.\n"
        report_text += "XGBoost provides improved performance over Random Forest through gradient boosting and\n"
        report_text += "better handling of class imbalance.\n"
...
"""
    ...
    def create_evaluation_report(self, all_results, summary_results):
        ...
        report = f"""XGBoost Ant Detection Model Evaluation Report
=======================================================
Generated: {timestamp}
